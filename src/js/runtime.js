...
(function () {
  ...
  class WebGLInterface {
    ...
    populateUniformTable(program) {
      ...
      for (
        let ptable = this.programInfos[program],
          ...
          numUniforms = this.ctx.getProgramParameter(
            p,
            this.ctx.ACTIVE_UNIFORMS,
          ),
          i = 0;
        i < numUniforms;
        ++i
    ...
    loadTypedArrayFromTexelType(type, size, data) {
      switch (type) {
        case this.ctx.SHORT:
          return this.mem.loadI16Array(data, size / 2);
        ...
        case this.ctx.HALF_FLOAT:
          return this.mem.loadU16Array(data, size / 2);
        case this.ctx.FLOAT:
          return this.mem.loadF32Array(data, size / 4);
        case this.ctx.INT:
          return this.mem.loadI32Array(data, size / 4);
        ...
        case this.ctx.UNSIGNED_INT:
          return this.mem.loadU32Array(data, size / 4);
        case this.ctx.BYTE:
          return this.mem.loadI8Array(data, size);
        case this.ctx.UNSIGNED_BYTE:
          return this.mem.loadU8Array(data, size);
        case this.ctx.FLOAT_32_UNSIGNED_INT_24_8_REV:
          throw new Error(
            "Source data of type FLOAT_32_UNSIGNED_INT_24_8_REV must be null",
          );
      }
    ...
    getWebGL1Interface() {
      return {
        ...
        GetSupportedExtensionFromIndex: (
          index,
          buf_ptr,
          buf_len,
          length_ptr,
          ...
          this.mem
            ...
            .set(new TextEncoder().encode(ext));
        ...
        BindRenderbuffer: (target, renderbuffer) => {
          this.ctx.bindRenderbuffer(
            target,
            renderbuffer ? this.renderbuffers[renderbuffer] : null,
          );
        },
        ...
        GetActiveAttrib: (
          ...
          if (name_buf_ptr && name_buf_len > 0) {
            ...
            this.mem
              .loadBytes(name_buf_ptr, name_buf_len)
        ...
        TexImage2D: (
          ...
          if (data) {
            const texelData = this.loadTypedArrayFromTexelType(
              type,
              size,
              data,
            );
    ...
    getWebGL2Interface() {
      return {
        ...
        GetActiveUniforms: (
          ...
          this.assertWebGL2();
          let indices = this.mem.loadU32Array(
            uniformIndices_ptr,
            uniformIndices_len,
          );
        ...
        GetActiveUniformBlockParameter: (
          ...
          this.assertWebGL2();
          let res = this.ctx.getActiveUniformBlockParameter(
            this.programs[program],
            uniformBlockIndex,
            pname,
          );
          ...
          if (res instanceof Uint32Array) {
            // for pname GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES
  ...
  function odinSetupDefaultImports(
    wasmMemoryInterface,
    consoleElement,
    memory,
  ) {
  ...
  async function runWasm(
    ...
    // run the main procedure from odin, as the main procedure is exported as _start
    if (exports._start) {
      exports._start();
    }

    // Define a `@export update :: proc(delta_time: f64) -> (keep_going: bool) {`
